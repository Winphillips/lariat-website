...
import bandImgSrc from '@/assets/band.webp'
import bandsolarImgSrc from '@/assets/bandsolar.webp'
import allheadsImgSrc from '@/assets/allheads.webp'
import lariatImgSrc from '@/assets/lariat.webp'
...
function cycleImages(member: Member) {
  ...
  member.intervalId = window.setInterval(() => {
    member.image = `${BASE}assets/${member.name}${count}.webp`
    count = (count % 7) + 1
  }, 200)
}
...
function stopCycle(member: Member) {
  ...
  member.image = `${BASE}assets/${member.name}.webp`
}
